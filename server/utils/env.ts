...
export const envSchema = z
  .object({
    ...
    S3_ENDPOINT: z.url(),
    // Optional: omit both to fall back to the AWS SDK default credential
    // provider chain (env vars → AWS_PROFILE → IRSA OIDC token → ECS task role
    // → EC2 IMDSv2). Required for self-hosted EKS via IRSA.
    S3_ACCESS_KEY: emptyToUndefined.pipe(z.string().min(1)).optional(),
    S3_SECRET_KEY: emptyToUndefined.pipe(z.string().min(1)).optional(),
     ...
     * Use this when the request path includes multiple proxies (e.g.
     * CloudFront → ALB → pod). The rate limiter pops trusted hops off the
     * right of X-Forwarded-For until it finds the first untrusted entry —
     * that's the real client. Either IPv4 or IPv6 CIDRs.
     */
    TRUSTED_PROXY_CIDRS: emptyToUndefined
      .pipe(z.string())
      .transform((v) =>
        v
          .split(",")
  ...
  .superRefine((data, ctx) => {
    ...
    // S3 credentials: both must be set together, or neither (IRSA / SDK default chain).
    if (Boolean(data.S3_ACCESS_KEY) !== Boolean(data.S3_SECRET_KEY)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: [data.S3_ACCESS_KEY ? "S3_SECRET_KEY" : "S3_ACCESS_KEY"],
        message:
          "S3_ACCESS_KEY and S3_SECRET_KEY must both be set or both be unset. Leave both unset to use the AWS SDK default credential provider chain (IRSA, EC2 instance profile, AWS_PROFILE).",
      });
    }
